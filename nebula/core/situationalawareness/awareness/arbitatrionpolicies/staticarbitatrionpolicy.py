...
class SAP(ArbitatrionPolicy): # Static Arbitatrion Policy
    ...
    async def tie_break(self, sac1: SACommand, sac2: SACommand) -> bool:
        ...
        """
        if self._verbose: logging.info(f"Tie break between ({await sac1.get_owner()}, {sac1.get_action().value}) & ({await sac2.get_owner()}, {sac2.get_action().value})")
        ...
        if await get_weight(sac1) > await get_weight(sac2):
            if self._verbose: logging.info(f"Tie break resolved, SA Command choosen ({await sac1.get_owner()}, {sac1.get_action().value})")
            return True
        else:
            if self._verbose: logging.info(f"Tie break resolved, SA Command choosen ({await sac2.get_owner()}, {sac2.get_action().value})") 
            return False
